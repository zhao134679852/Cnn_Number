...
def cnn_num(path):
    ...
    #     if i == 19999:
    #         saver.save(sess, "./tmp/model/number.ckpt")
    #
    # print("test accuracy %g" % accuracy.eval(feed_dict={
    #     x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0}))

    if os.path.exists("./tmp/model/checkpoint"):
        saver.restore(sess, "./tmp/model/number.ckpt")

    spath = dp.main(path)
    spath.rstrip(".jpg")
    # 转换为灰度图 28*28
    img = Image.open(spath + "_result.jpg").resize((28, 28), Image.ANTIALIAS).convert("L")
    flatten_img = np.reshape(img, 784)
    # 转换为array
    x_image = np.array([1 - flatten_img])
    # 会话运行
    result = sess.run(y_conv, feed_dict={x: x_image, keep_prob: 0.5})
    print("预测结果为：", sess.run(tf.argmax(result, 1)))


if __name__ == "__main__":
    cnn_num("./image/n7.jpg")
